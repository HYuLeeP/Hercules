...
def plot_period(ax,y0_periodic):
    # ax=axs[1]
...
#  main program  #
##################
def sos1k(y0,E_J,tmax=500): #we don't need ax now as we're not plotting
    '''
    input: y0,E_J,tmax=500
        y0      inital position
        E_J     Jacobi integral
        tmax    maximum time for integration
    output: vr_SNd, Lz_SNd
        vr_SNd  vr in physical coordinates (km/s)
        Lz_SNd  Lz in physical coordinates (kpc km/s)
    ...
    # if (max(x)<1.2):
    #     return np.empty, np.empty, y0 , E_J
    ...
    # plot the orbit
    # not plotting orbit for this modification
    ...
    # # plt.plot(-1.50719,0.,'bo', markersize = 3)
    # ax.axis([-axmax,axmax,-axmax,axmax])
    # ax.set_title(r'$y_0$ ='+' {:.1f}'.format(y0))
    # # plt.show()
    ...
    cdt_SNd = ((x-1.4)**2+(y-.8)**2 < 1/25)
    Lz_SNd = Lz_ph[cdt_SNd]
    vr_SNd = vr_ph[cdt_SNd]
    # # ax=axs[0]
    # # ax.plot(1.4,0.8,'r*',zorder=10,markersize=10)
    # # ax=axs[1]
    # # ax.plot(x[cdt_SNd],y[cdt_SNd],'lime')
    # # ax=axs[1]
    # ax.annotate('$E_J$ = {:.1f}'.format(E_J), xy=(0.2, 0.9), 
    #             xycoords='axes fraction', ha='center', va='center',
    #             bbox=dict(boxstyle='square', fc='w'),fontsize=12)
    # ax.set_xlim([-150,150])
    # ax.set_ylim([1000,2500])
    # ax.plot(vr_SNd,Lz_SNd,'g.',markersize=.5,zorder=10)

    return Lz_SNd, vr_SNd #returns vr and lz as arrays
